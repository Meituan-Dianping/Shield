...
class LegacyHeaderPaintingCallback(private val sci: SectionExtraCellInterface) : ViewPaintingCallback {
    override fun onCreateView(context: Context, parent: ViewGroup?, viewType: String?): View {
        //加在ViewItem上的callback不需要考虑viewtype的revert
        //但是Interface的转换过程中会加前缀 "headercell${NodeCreator.viewTypeSepreator}${sci.getHeaderViewType(section)}"
//        val originViewType = NodeCreator.revertViewType(viewType)
        return sci.onCreateHeaderView(parent, viewType?.toIntOrNull() ?: -1)
    }

    override fun updateView(view: View, data: Any?, path: NodePath?) {
        if (path?.cellType == CellType.HEADER) {
            sci.updateHeaderView(view, path.section, null)
        }
    }
    ...
    override fun equals(other: Any?): Boolean {
        ...
        other as LegacyHeaderPaintingCallback
